...
## 2026-07-11 — Seven-corpus V4 quality calibration

- **Input:** exact `Peacockery/farsi-asr-corpus-v4` revision `564d41da9e5b935c0fe2bf2443e205ca7b747c96`, seed 20260711, 200 deterministic train rows from one shard in each of the seven V4 corpora: 1,400 rows / 3.432 hours total. Runtime artifacts and exact shard/hash provenance live under gitignored `data/audit/v4-quality-calibration-20260711/`.
- **Independent draft ASR:** `C1Tech/whisper-base-fa`, model SHA256 `34b1b4d4f6c4325720ccbb09e5ab3060876fdff359688e24e863e31b4faef7ea`, selected from the persisted cross-model benchmark because its batch-1 greedy result is the best accessible WER: 18.17% WER / 7.03% CER on 2,101 shared rows. On this source-balanced V4 sample it produced 1,400/1,400 hypotheses with zero errors or empties at 19.29% corpus WER / 7.65% CER and 59.3× measured RTFx. C1Tech remains an audit comparator; its text is excluded from the teacher/label path.
- **ASR signals:** row-mean/median WER were 16.82% / 14.29%; 167 rows exceeded 35% WER, 69 exceeded 20% CER, 171 exceeded three boundary characters, and 17 exceeded ten boundary characters. The tails are source-dependent: edge-over-3 counts were 71 YouTube, 68 Thomcles, 23 WorldSpeech, and 0–5 in each remaining source. Review showed raw agreement over-penalizes digit-bearing labels because this Whisper checkpoint speaks numerals as words, while long-form edge tails can reflect recognizer truncation rather than missing label audio.
- **CTC signals:** pinned Persian CTC SHA256 `b11fc64a92e4cc457c2693356e85d296143b702bc8e25e2f7fe8ff485a4afa72` plus NeMo NFA commit `1d4ee423806d461f9146ae982f9da8eb32495ae7`. Tokenizer preflight aligned 1,399/1,400 rows and retained one WorldSpeech row as `token_case_incompatible` for the token `۱+۵`; the original label stayed unchanged. Median leading/trailing margins were 0.32/0.48 seconds and median aligned-span ratio was 0.870. Source format dominates the global tails: Common Voice contributed 180/276 span-ratio-below-0.70 rows, and FLEURS contributed 62/78 margins above two seconds.
- **Policy for the training gate:** preserve all raw signals and the one preflight rejection. Build 20-hour random and cleaned derivatives with identical source-hour counts and duration-bin counts. The cleaned arm ranks rows within each source/duration bin; digit-bearing rows omit the ASR-agreement component, edge mismatch contributes only for clips below 20 seconds, and alignment geometry contributes as a normalized within-bin distinct-value rank rather than a global cutoff. Sample the cleaned arm from the lower-risk half of each matched bin and the control arm randomly from the full bin. Promote a production filter only if the cleaned arm improves fixed held-out WER without a material source regression; otherwise keep the signals for review and retain V4.

## 2026-07-13 — V4 ASR-edge + CTC quality-filter training gate

- **Input and label provenance:** reused the 538,117 V4 rows and their published transcripts verbatim. This experiment ranked and selected existing rows; it generated zero replacement labels. The full ledger contains 538,117 ASR-quality records and 538,117 CTC-alignment records, with zero ASR scoring errors, 538,071 aligned rows, and 46 unaligned rows.
- **Deterministic export:** `farsi-v4-quality export-ablation` joins the restored embedded-audio V4 Parquets to the durable quality ledger, verifies transcript equality and audio SHA256 while materializing clips, stores the exact selection back into SQLite, and emits stable Parakeet manifests. This is the narrow V4 adapter around the existing curator/export infrastructure; it avoids rebuilding or relabeling the corpus.
- **Matched arms:** run `v4-quality-20h-s20260713-v2`, seed 20260713, seven sources balanced equally and each source's four real duration bins preserved. Control and cleaned each contain 9,788 clips and 19.9995 hours, overlap is zero, and the largest per-source duration difference is 0.48 seconds. Mean composite risk is 0.15750 control versus 0.09027 cleaned. Control manifest SHA256 `c914d88be68c59ca61038c68cf85bdf4c80dcae9e2c813d9f9c5cd438dcc49b6`; cleaned manifest SHA256 `75ab38b8478d5d0f09c7505eb5f72c2cd216cd39cda9ac7df2a7f5d25cf466f8`.
- **Training:** 110M hybrid base with pure TDT loss, BPE-512 V4 tokenizer, AdamW, lr 5e-4, batch duration 120 seconds, 200 warmup steps, 2,000 total steps, SpecAugment off, seeds 0 and 1. `compute_eval_loss=True`; best checkpoints were selected by validation loss and evaluated in fp32 on the fixed six-surface suite. Integer-step validation reset at each short data epoch, so the best checkpoints landed at steps 1,754–1,757; paired arms remain effectively step matched.

| Surface           | Control s0 | Cleaned s0 | Delta s0 | Control s1 | Cleaned s1 | Delta s1 | Two-seed delta |
| ----------------- | ---------: | ---------: | -------: | ---------: | ---------: | -------: | -------------: |
| FLEURS dev        |      38.22 |      37.92 |    -0.29 |      37.45 |      37.59 |    +0.15 |          -0.07 |
| YouTube dev conv  |      53.47 |      53.43 |    -0.04 |      52.10 |      52.35 |    +0.24 |          +0.10 |
| FLEURS test       |      38.26 |      36.57 |    -1.69 |      37.41 |      37.03 |    -0.39 |          -1.04 |
| Neyshekar test    |      41.42 |      41.34 |    -0.08 |      41.28 |      41.79 |    +0.51 |          +0.21 |
| WorldSpeech test  |      51.89 |      52.62 |    +0.73 |      51.61 |      52.41 |    +0.80 |          +0.76 |
| YouTube test conv |      53.02 |      52.88 |    -0.14 |      51.57 |      51.95 |    +0.38 |          +0.12 |
| Six-surface macro |      46.05 |      45.79 |    -0.25 |      45.24 |      45.52 |    +0.28 |          +0.02 |
