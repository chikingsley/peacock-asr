...
shards → verify them on the Hub → delete the local archive *and* the local shards.** The shards
live on the Hub, so nothing data-bearing accumulates locally; adding a language adds shard files to
the Hub, nothing is rewritten.
...
Secrets come from the environment or an ignored `.env` (repo root or this dir). Never paste keys
into commands, tmux logs, or committed files.
...
`download` and `process` run concurrently under `--watch`: the downloader lands archives while the
processor drains them, keeping local disk bounded. `process` without `--upload` just builds parquet
locally; `--delete-after-upload` only removes an archive after its shards verify on the Hub.
...
MDC archives use the standard Common Voice layout (`train/dev/test/validated/invalidated/other.tsv`,
`clip_durations.tsv`, `clips/*.mp3`); the ASR TSVs share one header. Splits may be empty — the
upstream split name is preserved in `upstream_split`, and only non-empty splits produce a shard at
`data/<upstream_split>/<collection>__<locale>__<dataset_id>.parquet`.
...
- upstream Common Voice columns: `client_id`, `sentence_id`, `sentence_domain`, `up_votes`,
  `down_votes`, `age`, `gender`, `accents`, `variant`, `segment`
