# P011 Implementation Notes

## Status note

P011 is the isolated branch for **paper-faithful frame-level HConv** on top of MuFFIN.
The live experiment record is [EXPERIMENT_LOG.md](./EXPERIMENT_LOG.md). Large parts of
this file were copied forward from P010 and remain useful as historical background for
MuFFIN/ConPCO replication, but any inherited discussion of phone-level HConv/CHConv is
historical only and should not be treated as the current P011 experiment definition.
...
```bash
# Pretrain (run once)
uv run p011 pretrain

# Full MuFFIN sweep
uv run p011 sweep --seeds 22,33,44,55,66 --use-conpco --use-mdd --use-phnvar \
    --pretrained checkpoints/pretrained/best_pretrain.pth

# Evaluate with threshold search
uv run p011 eval --checkpoint checkpoints/v3-muffin/seed22/best_model.pth --use-mdd
```
...
| `p011 extract` CLI command | No longer needed. |
...
uv run p011 train --ssl-interface hconv --ssl-models hubert --batch-size 5 --grad-accum-steps 5

# Branch-preserving one-stream HConv ablation
uv run p011 train --ssl-interface hconv --ssl-models hubert --ssl-output-dim 3072 \
...
| Real data in all tests | All tests use the `features_dir` session fixture from `conftest.py`, which reads `P011_FEATURES_DIR` from `.env` via `Settings()`. No synthetic/mocked data anywhere. If the data is absent, the fixture downloads it automatically; if `P011_FEATURES_DIR` is not configured, tests fail clearly via `pytest.fail()` (not `pytest.mark.skipif`). |
| Two `.env` files | Root `.env` (`~/github/peacock-asr/.env`) holds API keys (HF_TOKEN, WANDB_API_KEY, etc.) loaded by shell and library init. Project `.env` (`projects/P011-muffin-improvements/.env`) holds `P011_FEATURES_DIR`, loaded by pydantic-settings inside `Settings()`. Separation keeps API secrets out of the project directory. |
...
Initial HConv and CHConv training runs are running locally on RTX 5070 using phone-level all-layer features. Results pending — see W&B project `p011-muffin` for live metrics.
