...
Requires P011_FEATURES_DIR to be set. Tests fail (not skip) if data is absent.
...
def test_dataset_shapes(features_dir: Path) -> None:
    from p011.data import GoPDataset
    from p011.ssl_features import ssl_feature_dim
...
def test_ssl_concat_shape(features_dir: Path) -> None:
    ...
    from p011.data import GoPDataset
    from p011.ssl_features import SSL_MODEL_KEYS, ssl_feature_dim
