...
def test_chconv_output_shape() -> None:
    ...
    from p011.models.hconv import CHConv
...
def test_frame_hconv_interface_total_projection() -> None:
    """FrameHConvInterface can be configured to hit a total SSL output size."""
    from p011.models.ssl_interface import FrameHConvInterface

    interface = FrameHConvInterface(ssl_output_dim=3072)
    assert interface.output_dim == 3072


def test_frame_hconv_interface_single_model_projection() -> None:
    """A one-model HConv interface can preserve the upstream width via projection."""
    from p011.models.ssl_interface import FrameHConvInterface

    interface = FrameHConvInterface(ssl_output_dim=1024, ssl_model_keys=("hubert",))
