...
One-off enrichment pilot that fetched recent video titles per channel (`yt-dlp
--flat-playlist`) and asked the LLM for a channel-level genre plus a per-video genre,
...
`@`-handle was wrong; resolved to a working channel-id / `/c/` / `/user/` URL in
`sources.py`:

- tamadon_tv, hasht_e_subh, etilaatroz, voa_dari, azadi_radio, begum_tv, rukhshana_media

Still unresolved (left as `@FarhadDaryaOfficial`): **farhad_darya** — music-only channel,
low spoken-ASR value; revisit only if a working URL is found.

## Channel category distribution (19 live channels)

news_bulletin 7 · vlog_lifestyle 6 · panel_talkshow 2 · podcast 2 · documentary 1 ·
educational_explainer 1
...
Takeaway: channel-level genre is a weak prior for several channels; the curator prescan
should score genre per-video, not per-channel.
