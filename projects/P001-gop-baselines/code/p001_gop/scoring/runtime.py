...
from p001_gop.scoring.prepared_bundle import (
    PreparedItem,
...
if TYPE_CHECKING:
    ...
    from p001_gop.dataset import Utterance
    from p001_gop.evaluate import EvalResult
    from p001_gop.gop import GOPResult, ScalarGOPResult
    from p001_gop.gopt_model import UtteranceFeats
    from p001_gop.score_variants import ScoreVariant
...
def _scalar_gop_worker(
    ...
    from p001_gop.gop import compute_gop_scalar  # noqa: PLC0415
...
def _collect_split_outputs(
    *,
    ...
    from p001_gop.gop import (  # noqa: PLC0415
        GOPResult,
        ScalarGOPResult,
        compute_gop_features,
    )
    from p001_gop.gopt_model import UtteranceFeats  # noqa: PLC0415
    from p001_gop.score_variants import apply_score_variant  # noqa: PLC0415
    ...
    for idx, (post, pidx, valid_phones, valid_scores) in enumerate(iterator):
        ...
        if n_workers <= 1:
            ...
            if not (
                isinstance(gop, ScalarGOPResult)
                or (
                    hasattr(gop, "ll_self")
                    and hasattr(gop, "scores")
                    and hasattr(gop, "occupancies")
                )
            ):
                msg = "Expected ScalarGOPResult-compatible value in parallel mode."
                raise TypeError(msg)
            ll_self = float(cast("int | float", gop.ll_self))
            features = compute_gop_features(
                post, pidx, ll_self, blank, device=device,
            )
...
def _process_split(  # noqa: PLR0913
    *,
    ...
    from p001_gop.dataset import DATASET_REVISION  # noqa: PLC0415
    from p001_gop.gop import compute_gop  # noqa: PLC0415
    ...
    if use_parallel_scalar:
        ...
        scalar_gop_results_pending: list[ScalarGOPResult | None] = [
            None for _ in worker_args
        ]
        with ProcessPoolExecutor(max_workers=n_workers) as pool:
            ...
            try:
                for future in as_completed(futures):
                    ...
                    scalar_gop_results_pending[index] = future.result()
                    progress.update(1)
            finally:
                progress.close()
        scalar_gop_results = cast(
            ...
            [
                result for result in scalar_gop_results_pending if result is not None
            ],
        )
    else:
        scalar_gop_results = []
        for post, pidx, _, _ in tqdm(
            ...
            if extract_features:
                ...
                from p001_gop.gop import compute_gop_scalar_scores_only  # noqa: PLC0415
...
def _wandb_tags(*base_tags: str | None) -> list[str]:
    ...
    return [
        tag
        for tag in dict.fromkeys([*base_tags, *extra_tags])
        if tag
    ]
    ...
    return _with_git_sha(
        {
            ...
            "checkpoint_dir": (
                ...
                else str(payload.checkpoint_dir)
            ),
        },
    )
...
def _maybe_upload_checkpoint_to_hf(checkpoint_dir: Path) -> None:
    from p001_gop.hf_checkpoints import upload_checkpoint_folder  # noqa: PLC0415
    from p001_gop.settings import settings  # noqa: PLC0415
...
def _log_eval_to_wandb(payload: _EvalWandbPayload) -> None:
    ...
    _log_per_phone_pcc_table(run, wandb, payload.eval_result.per_phone_pcc)
    _log_training_history_to_wandb(run, payload.training_history)
...
def cmd_run(args: argparse.Namespace) -> tuple[str, EvalResult]:  # noqa: PLR0912,PLR0915
    ...
    from p001_gop.backend_loader import get_backend  # noqa: PLC0415
    from p001_gop.dataset import (  # noqa: PLC0415
        DATASET_REVISION,
        load_speechocean762,
    )
    from p001_gop.settings import settings  # noqa: PLC0415
    ...
    use_hmamba = getattr(args, "hmamba", False)
    ...
    if use_gopt and use_hmamba:
        msg = "Choose at most one contextual scorer: --gopt or --hmamba."
        raise ValueError(msg)
    if score_variant != "gop_sf" and (use_feats or use_gopt or use_hmamba):
        msg = (
            "score variants other than 'gop_sf' currently support scalar mode "
            "only (no --feats/--gopt/--hmamba)."
        )
        raise ValueError(msg)
    extract_features = use_feats or use_gopt or use_hmamba
...
def cmd_prewarm_k2(args: argparse.Namespace) -> None:
    """Populate the k2 topology cache for one backend and split selection."""
    from p001_gop.backend_loader import get_backend  # noqa: PLC0415
    from p001_gop.dataset import (  # noqa: PLC0415
        DATASET_REVISION,
        load_speechocean762,
    )
    from p001_gop.k2_scalar import (  # noqa: PLC0415
        k2_available,
    ...
    for split_name in split_names:
        ...
        if prepared_cache is not None:
            ...
            logger.info("[%s] Saved prepared-input cache", split_name)
        start = perf_counter()
        stats = prewarm_topology_cache(prepared, blank=backend.blank_index)
...
def _run_evaluation(  # noqa: PLR0913
    *,
    ...
    from p001_gop.evaluate import (  # noqa: PLC0415
        EvalResult,
        evaluate_gop,
        evaluate_gop_feats,
    )
    ...
    if use_hmamba:
        from p001_gop.hmamba_model import (  # noqa: PLC0415
            train_and_evaluate_hmamba,
        )
        ...
        logger.info("Training HMamba scorer (feat_dim=%d)...", feat_dim)
        training_history = []
        eval_result = train_and_evaluate_hmamba(
            train_utts,
            test_utts,
            input_dim=feat_dim,
            device=device,
        ...
        eval_name = backend_name + " (HMamba)"
        _print_results(eval_name, eval_result, verbose)
    elif use_gopt:
        from p001_gop.gopt_model import (  # noqa: PLC0415
...
def _print_results(
    ...
    from p001_gop.evaluate import EvalResult  # noqa: PLC0415
...
def cmd_sweep_alpha(args: argparse.Namespace) -> None:  # noqa: PLR0915
    ...
    from p001_gop.backend_loader import resolve_backend_name  # noqa: PLC0415
    from p001_gop.dataset import DATASET_REVISION  # noqa: PLC0415
    from p001_gop.evaluate import evaluate_gop  # noqa: PLC0415
    from p001_gop.settings import settings  # noqa: PLC0415
