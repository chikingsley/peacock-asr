...
class ZIPABackend:
    ...
    def get_posteriors(self, audio: np.ndarray, sample_rate: int) -> np.ndarray:
        ...
        extractors = importlib.import_module("lhotse.features.kaldi.extractors")
        extractor = extractors.Fbank(
            extractors.FbankConfig(num_filters=80, dither=0.0, snip_edges=False),
        )
