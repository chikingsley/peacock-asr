...
BatchMode = Literal["scalar", "feats", "gopt", "hmamba"]
...
class BatchSpec(BaseModel):
    ...
    @classmethod
    def _validate_jobs_non_empty(
        cls, value: list[BatchJob],
    ) -> list[BatchJob]:
        del cls
        if len(value) == 0:
...
def _resolve_seed_plan(
    *,
    ...
    return repeats, [None for _ in range(repeats)]
