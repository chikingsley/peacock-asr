...
Adapted from the vendored P001 CTC-based-GOP reference:
`projects/P001-gop-baselines/third_party/CTC-based-GOP/taslpro26/gop_sf_sd_norm.py`
...
def compute_gop(
    ...
    *,
    ...
    ll_self, scores, occupancies = _compute_scalar_terms(
        posteriors,
        phone_indices,
        blank,
    )
...
def _compute_scalar_terms_python(
    params: torch.Tensor,
    seq: torch.Tensor,
    blank: int,
...
def _compute_scalar_terms(
    ...
    if settings.ctc_scalar_backend == "k2":
        from p001_gop.k2_scalar import (  # noqa: PLC0415
            compute_scalar_terms_k2,
            k2_available,
        )

        if k2_available():
            return compute_scalar_terms_k2(
                posteriors,
                phone_indices,
                blank=blank,
        ...
        global _K2_FALLBACK_WARNED  # noqa: PLW0603
        if not _K2_FALLBACK_WARNED:
            logger.warning(
                "ctc_scalar_backend='k2' requested but k2 is unavailable; "
                "falling back to the Python scalar path."
            )
            _K2_FALLBACK_WARNED = True
...
def compute_gop_scalar_scores_only(
    posteriors: np.ndarray,
    phone_indices: list[int],
    blank: int = 0,
) -> ScalarGOPResult:
    """Compute scalar GOP scores without occupancy bookkeeping.

    This is the relevant path for phase-2 scalar scoring, where occupancies are
    ...
    if settings.ctc_scalar_backend == "k2":
        from p001_gop.k2_scalar import (  # noqa: PLC0415
            compute_scalar_scores_k2,
            k2_available,
        )

        if k2_available():
            ll_self, scores = compute_scalar_scores_k2(
                posteriors,
                phone_indices,
                blank=blank,
                device=settings.scalar_torch_device,
    ...
    ll_self, scores, _occupancies = _compute_scalar_terms(
        posteriors,
        phone_indices,
        blank,
    )
    return ScalarGOPResult(ll_self=ll_self, scores=scores, occupancies=[])
