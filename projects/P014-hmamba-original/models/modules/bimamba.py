...
# Not used in forward() — only needed for the un-fused path
causal_conv1d_fn, causal_conv1d_update = None, None
mamba_inner_fn, bimamba_inner_fn, mamba_inner_fn_no_out_proj = None, None, None

# Triton ops not imported — they trigger long JIT compilation on Blackwell (sm_120)
# selective_state_update is only used in step() (inference), not forward()
selective_state_update = None
# RMSNorm/layer_norm_fn/rms_norm_fn only used by the Block class at the bottom of this
# file, which is NOT used by HMamba (HMamba defines its own Block in hmamba.py)
...
class Mamba(nn.Module):
    def __init__(
        ...
        conv_bias=True,
        bias=False,
        use_fast_path=False,  # Fused kernel options (disabled for causal-conv1d >=1.2 compat)
        layer_idx=None,
        device=None,
        dtype=None,
        bimamba_type="none",
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            ...
            if self.bimamba_type == "v2":
                ...
                y_b = selective_scan_fn(
                    ...
                    z=z_b, delta_bias=self.dt_proj_b.bias.float(), delta_softplus=True,
                )
                if self.if_devide_out:
                    y = 0.5 * y + 0.5 * y_b.flip([-1])
                else:
                    y = y + y_b.flip([-1])
