"""Curate Tajik ASR data through omni-curator. Thin wiring of sources.py into the curator stages.

  tajik-curate list                       # size the configured YouTube channels (no download)
  tajik-curate download [--tier clean]    # pull channel audio -> data/create/<slug>  (create stage)
  tajik-curate download --channel asiaplus --limit 30
  tajik-curate ingest fleurs              # google/fleurs tg_tj -> the curator store
  tajik-curate ingest commonvoice         # Common Voice via MDC (needs ids in sources.COMMONVOICE)

The curator store (``data/curator.sqlite``) is the master pool; every source funnels through it.
Downloads land as 16 kHz mono FLAC (resumable). Labeling channel audio -> store and verify/export
are the next stages. All logic is in omni-curator; this only supplies the Tajik sources.
"""
...
def _load_root_env() -> None:
    """Load KEY=VALUE lines from the monorepo-root .env (MDC_API_KEY, HF_TOKEN, ...)."""
...
def cmd_ingest(args: argparse.Namespace) -> int:
    ...
    if args.dataset == "fleurs":
        ...
        if not sources.COMMONVOICE:
            store.close()
            raise SystemExit("no Common Voice dataset ids in sources.COMMONVOICE yet")
