...
class HeadWERPrinter(pl.Callback):
    def on_validation_epoch_end(self, trainer: pl.Trainer, _pl: pl.LightningModule) -> None:
        ...
        print(f"  [val @ step {trainer.global_step}]  TDT val_wer={g('val_wer'):.4f}  "
              f"CTC val_wer_ctc={g('val_wer_ctc'):.4f}", flush=True)
...
def main() -> int:
    ...
    print(f"=== arm {args.arm}  steps {args.steps} ===", flush=True)
    ...
    cfg = OmegaConf.create({"model": {
        "train_ds": ds(TRAIN, args.bs, shuffle=True),
        "validation_ds": ds(DEV, args.bs, shuffle=False),
