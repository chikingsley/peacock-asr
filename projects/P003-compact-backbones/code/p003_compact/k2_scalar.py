...
if TYPE_CHECKING:
    ...
    from p003_compact.scoring.prepared_bundle import PreparedItem
...
def _load_cached_topology(path: Path) -> DenomTopology | None:
    ...
    return DenomTopology(
        ...
        arb_logical_state=int(data["arb_logical_state"]),
...
def _save_cached_topology(path: Path, topology: DenomTopology) -> None:
    payload = {
        ...
        "arb_logical_state": topology.arb_logical_state,
...
def _occupancies_from_pruned_lattice(
    ...
    frame_lengths = torch.tensor(
        [topology.frame_state_ids.shape[0] for topology in topologies],
        dtype=torch.long,
        device=arc_forward.device,
    )
    arb_states = torch.tensor(
        [topology.arb_logical_state for topology in topologies],
        dtype=torch.long,
        device=arc_forward.device,
    )
    max_frames = int(frame_lengths.max().item()) if len(topologies) > 0 else 0
    if max_frames == 0:
        return [0.0 for _ in topologies]

    valid_frame_mask = dest_frames < frame_lengths[case_ids_per_arc]
    if not bool(valid_frame_mask.any()):
        return [0.0 for _ in topologies]

    case_ids = case_ids_per_arc[valid_frame_mask]
    frame_ids = dest_frames[valid_frame_mask]
    values = arc_forward[valid_frame_mask]
    group_ids = case_ids * max_frames + frame_ids
    num_groups = len(topologies) * max_frames

    total_log = _segment_logsumexp(
        values,
        group_ids,
        num_groups=num_groups,
    )

    arb_mask = dest_logical_states[valid_frame_mask] == arb_states[case_ids]
    if not bool(arb_mask.any()):
        return [0.0 for _ in topologies]

    arb_group_ids = group_ids[arb_mask]
    arb_log = _segment_logsumexp(
        values[arb_mask],
        arb_group_ids,
        num_groups=num_groups,
    )

    arb_valid = torch.isfinite(arb_log)
    frame_occ = torch.zeros(
        num_groups,
        dtype=arc_forward.dtype,
        device=arc_forward.device,
    )
    frame_occ[arb_valid] = torch.exp(arb_log[arb_valid] - total_log[arb_valid])

    case_occ = frame_occ.view(len(topologies), max_frames).sum(dim=1)
    return [float(value) for value in case_occ.detach().cpu().tolist()]
...
def _segment_logsumexp(
    ...
    *,
    ...
    exp_values = torch.exp(values - max_values[group_ids])
    sum_values = torch.zeros(
        (num_groups,),
        dtype=values.dtype,
        device=values.device,
    )
    sum_values.scatter_add_(0, group_ids, exp_values)
    ...
    result[valid] = torch.log(sum_values[valid]) + max_values[valid]
    return result


def _python_scalar_fns() -> tuple[Any, Any]:
    module = importlib.import_module("p003_compact.gop")
    return module._compute_scalar_terms_python, module._ctc_forward  # noqa: SLF001


def _log_probs_batch_for_cases(
    cases: Sequence[tuple[np.ndarray, Sequence[int]]],
    *,
    device: torch.device,
) -> tuple[torch.Tensor, torch.Tensor]:
    max_frames = max(posteriors.shape[0] for posteriors, _ in cases)
    vocab_size = cases[0][0].shape[1]
    batch = torch.full(
        (len(cases), max_frames, vocab_size),
        fill_value=float("-inf"),
        dtype=torch.float32,
        device=device,
    )
    input_lengths = torch.empty(len(cases), dtype=torch.int32)

    for idx, (posteriors, _) in enumerate(cases):
        frames = posteriors.shape[0]
        input_lengths[idx] = frames
        log_probs = torch.log(
            torch.from_numpy(posteriors).clamp(min=1e-30),
        ).to(device=device, dtype=torch.float32)
        batch[idx, :frames, :] = log_probs

    return batch, input_lengths


def _compute_ll_selfs_ctc_batch(
    cases: Sequence[tuple[np.ndarray, Sequence[int]]],
    *,
    blank: int,
    device: torch.device,
) -> list[float]:
    log_probs_batch, input_lengths = _log_probs_batch_for_cases(cases, device=device)
    max_target_len = max(len(phone_indices) for _, phone_indices in cases)
    targets = torch.zeros(
        (len(cases), max_target_len),
        dtype=torch.long,
        device=device,
    )
    target_lengths = torch.empty(len(cases), dtype=torch.long, device=device)

    for idx, (_, phone_indices) in enumerate(cases):
        target_lengths[idx] = len(phone_indices)
        if phone_indices:
            targets[idx, : len(phone_indices)] = torch.tensor(
                list(phone_indices),
                dtype=torch.long,
                device=device,
            )
    ...
    losses = functional.ctc_loss(
        log_probs_batch.transpose(0, 1),
        targets,
        input_lengths.to(device=device, dtype=torch.long),
        target_lengths,
        blank=blank,
        reduction="none",
        zero_infinity=True,
...
def compute_scalar_terms_k2_batch(
    cases: Sequence[tuple[np.ndarray, Sequence[int]]],
    *,
    blank: int = BLANK,
    device: torch.device | None = None,
    use_ctc_self: bool = False,
    ...
    for posteriors, _phone_indices in cases:
        if posteriors.ndim != _POSTERIOR_MATRIX_NDIM:
    ...
    if use_ctc_self:
        ...
        for posteriors, phone_indices in cases:
            ...
            ll_selfs.append(_ctc_forward(params, seq, blank=blank))
    log_probs_batch, _input_lengths = _log_probs_batch_for_cases(
        cases,
        device=device,
    )
