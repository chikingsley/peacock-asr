"""Tests for the P002 GOPT + ConPCO scoring stack."""
...
from p002_conpco.gopt_track09 import (
    GOPTConPCOModel,
    train_and_evaluate_gopt_conpco,
)
from p002_conpco.phones import ARPABET_VOCAB
...
class TestGOPTModelConPCO:
    ...
    def test_return_projections_tuple(self):
        ...
        model = GOPTConPCOModel(input_dim=42, embed_dim=24, use_conpco=True)
        x = torch.randn(2, SEQ_LEN, 42)
        phn = torch.randint(0, 39, (2, SEQ_LEN))
    ...
    def test_no_conpco_backward_compatible(self):
        ...
        model = GOPTConPCOModel(input_dim=42, embed_dim=24, use_conpco=False)
    ...
    def test_projection_param_count(self):
        ...
        model_base = GOPTModel(input_dim=42, embed_dim=24)
        model_conpco = GOPTConPCOModel(input_dim=42, embed_dim=24, use_conpco=True)
        base_params = sum(p.numel() for p in model_base.parameters())
        conpco_params = sum(p.numel() for p in model_conpco.parameters())
        # Each MLP: Linear(24,24) = 24*24+24 = 600, twice = 1200 per head,
        # 2 heads = 2400.
        expected_extra = 2 * (24 * 24 + 24 + 24 * 24 + 24)  # 2 x (600+600) = 2400
