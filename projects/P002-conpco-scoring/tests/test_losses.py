...
class TestCLAPContrastiveLoss:
    ...
    def test_padding_excluded(self):
        ...
        phn_ids_p = torch.cat(
            [phn_ids, torch.full((2, 5), -1, dtype=torch.long)],
            dim=1,
        )
    ...
    def test_identical_centers_low_loss(self):
        ...
        # Explicitly swap the phoneme blocks so the audio/text centers are
        # misaligned in a deterministic way.
        text_swapped = torch.cat([shared[:, 5:], shared[:, :5]], dim=1)
        loss_swapped = loss_fn(
            shared.clone().requires_grad_(True),
            text_swapped.clone().requires_grad_(True),
            scores, phn_ids,
        )

        assert loss_same.item() < loss_swapped.item()
