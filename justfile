...
# Validate or upload the local L2-ARCTIC release to Hugging Face.
# Dry-run by default; append --execute to actually publish.
hf-upload-l2arctic repo_id *args:
...
# Dry-run by default; append --execute to publish.
hf-prepare-l2arctic-manual repo_id *args:
    uv run scripts/prepare_l2arctic_manual_16k.py \
        --repo-id {{repo_id}} \
        --source-dir /Users/chiejimofor/Downloads/l2arctic_release_v5.0 \
        {{args}}
