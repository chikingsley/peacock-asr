...
This installs standard data libraries (`pyarrow`, `polars`, `pandas`) for data loading. For data preparation with HuggingFace datasets, you'll also need:
...
1. Perform weighted data mixture sampling across corpus-language combinations
1. Load specific languages for evaluation (`split=dev` and `language is in ["deu_Latn", "fra_Latn"]`)
...
1. Create the dataset (which will scan all files and organize them by corpus and language)
1. Create a data reader using task_config and storage_config
1. Iterate through a few batches and show statistics
