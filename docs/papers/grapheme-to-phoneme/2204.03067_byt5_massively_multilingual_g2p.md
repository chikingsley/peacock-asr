...
In order to create a large pronunciation dataset, we manually selected and merged several publicly available G2P datasets [17, 23] and constructed G2P mapping dictionaries for some new languages. To promote future research, we released a detailed catalogue of available G2P data and their licenses at: .
...
G2P is a sequence transduction task in which an input orthographic sequence is transformed to an output phoneme sequence through a mapping function , which is a neural network in this study. To tackle challenges in multilingual G2P, we implemented neural models with the following considerations.

Byte-level encoding For language modeling, the common approach to encode texts is to employ the byte pair encoding (BPE) to segment raw texts into subword units [31], which are then mapped to a set of embeddings. However, for multilingual models, BPE tokenizaation could potentially result in a huge vocabulary size, inflating model parameters. In contrast, token-free models [32] operating on raw bytes could process any type of strings, making multilingual processing easy and reduces the complicated text processing pipeline. The trade-off is that converting raw texts to bytes generally result in longer input sequences, causing a slowdown in training and inference, especially for transformers that have a time complexity of . Yet byte-level encoding has been shown to benefit the multilingual G2P for at least a few languages [18].
...
| mT5-tiny - 12 layers | 17.1 | 48.5 | 139M | 2063 words/second |
| ByT5-tiny - 8 layers | 9.8 | 28.8 | 7.3M | 2475 words/second |
| ByT5-tiny - 12 layers | 9.7 | 28.3 | 15.9M | 1833 words/second |
| ByT5-tiny - 16 layers | 9.5 | 27.7 | 20.6M | 1706 words/second |

## 4 Experiments

We trained a number of multilingual and monolingual models to perform G2P. All pretrained models, including their hyperparameters and training states, are available on the project website111. For multilingual models, we fine-tuned the pretrained ByT5-small and mT5-small on all language data. As baselines, we also trained lightweight version of the ByT5 model with 8, 12 and 16 layers. All models were trained on an A40 GPU with 48GB of memory. We set the effective batch size to 512 and the learning rate to using the AdamW optimizer [36]. All models were trained for 10 epochs and the validation PER was used to select final models. Generally, training multilingual models took about 0.8 to 2.5 days depending on the model size.

For comparison, we also trained two set of monolingual ByT5 models for each language on the same hardware. One set of models was trained with randomly initialized weights and the other set was fine-tuned from a pretrained multilingual model. All of these monolingual models were based on the same underlying architecture (ByT5-tiny-8 layers). All hyperparameters were the same as the multilingual models except that the batch size was set to 32. While this set of hyperparameters might not be optimal for all languages, we tried to make the comparison fair as both multilingual and monolingual models have seen the same amount of data for ten epochs. The training time for monolingual models ranged from less than 20 minutes for low-resource languages and more than 3 hours for high resource languages.

| Model | Language | ||||
| Tibetan | Albanian | Hausa | Hebrew | Lower Sorbian | |
| Zero-shot G2P | |||||
| mT5-small | 155.6 / 100 | 53.4 / 96 | 77.3 / 100 | 110.1 / 100 | 38.8 / 89.5 |
| ByT5-small | 109.3 / 100 | 56.1 / 99 | 76.9 / 100 | 101.8 / 100 | 42.7 / 89 |
| ByT5-tiny - 8 layers | 113.4 / 100 | 52.8 / 99 | 75.9 / 100 | 137.8 / 100 | 45.1 / 93 |
| Finetuning | |||||
| ByT5-small (text pretraining) | 95.2 / 100 | 12.3 / 45 | 25.4 / 78.5 | 36.6 / 89 | 7.6 / 35 |
| ByT5-small (G2P pretraining) | 51.7 / 79 | 11.7 / 43.5 | 25.9 / 76.5 | 36.1 / 84.5 | 7.8 / 34.5 |
| ByT5-tiny - 8 layers (G2P pretraining) | 56 / 76.5 | 12.2 / 45 | 25.8 / 80 | 37.3 / 86.5 | 6.4 / 30.5 |
| ByT5-tiny - 8 layers (random initialization) | 73.2 / 94 | 51.1 / 90 | 62.6 / 97 | 58.5 / 94.5 | 31.2 / 74.5 |
...
We analyzed the relationship between the data size and the final performance (Fig 3). For mT5, data size has a moderate negative correlation with the final performance (Spearman’s : -0.45), as more data tend to lower the PER in that language. However, there is almost no correlation between data size and PER for multilingual ByT5 models (Spearman’s : -0.05). For monolingual models, PER is negatively correlated to the size of dictionary (Spearman’s : -0.64) if these models are trained from randomly initialized weights. Yet if these monolingual models are fine-tuned, the correlation is almost non-existent (Spearman’s : -0.10).
...
Given that zero-shot prediction does not always generalize, we tested whether fine-tuning pretrained multilingual models can further reduce PER. Specifically, we compared whether weights from a pretrained text model (the original ByT5-small) and weights from pretrained G2P model can benefit low resource G2P. Even with only about 1800 training words in each language, finetuning can successfully reduce both PER and WER by a significant margin. If the model was initialized with ByT5-small pretrained weights, not only did the model generally converged to higher PER and WER but also took more iterations to train. However, for low resource languages, the training dynamics of G2P models was very unstable, with PER rate varying greatly at successive checkpoints and susceptible to overfitting.
...
conversion,”
*Speech communication*, vol. 50, no. 5, pp. 434–451, 2008. -
[2]
...
in
*Proceedings of the 10th International Workshop on Finite State Methods and Natural Language Processing*, Jul. 2012. -
[3]
K. Rao, F. Peng, H. Sak, and F. Beaufays, “Grapheme-to-phoneme conversion
using long short-term memory recurrent neural networks,” in
*2015 IEEE International Conference on Acoustics, Speech and Signal Processing (ICASSP)*. IEEE, 2015, pp. 4225–4229. -
[4]
K. Yao and G. Zweig, “Sequence-to-sequence neural net models for
grapheme-to-phoneme conversion,” in
*Proc. Interspeech 2015*, 2015, pp. 3330–3334. -
[5]
S. Toshniwal and K. Livescu, “Jointly learning to align and convert graphemes
to phonemes with neural attention models,” in
*2016 IEEE Spoken Language Technology Workshop (SLT)*. IEEE, 2016, pp. 76–82. -
[6]
A. E.-D. Mousa and B. W. Schuller, “Deep bidirectional long short-term memory
recurrent neural networks for grapheme-to-phoneme conversion utilizing
complex many-to-many alignments.” in
*Interspeech*, 2016, pp. 2836–2840. -
[7]
B. Milde, C. Schmidt, and J. Köhler, “Multitask sequence-to-sequence
models for grapheme-to-phoneme conversion.” in
*INTERSPEECH*, 2017, pp. 2536–2540. -
[8]
S. Yolchuyeva, G. Németh, and B. Gyires-Tóth, “Transformer Based
Grapheme-to-Phoneme Conversion,” in
*Proc. Interspeech 2019*, 2019, pp. 2095–2099. -
[9]
H. Sun, X. Tan, J.-W. Gan, H. Liu, S. Zhao, T. Qin, and T.-Y. Liu,
“Token-Level Ensemble Distillation for Grapheme-to-Phoneme Conversion,”
in
*Proc. Interspeech 2019*, 2019, pp. 2115–2119. [Online]. Available: -
[10]
K. Park and S. Lee, “g2pM: A Neural Grapheme-to-Phoneme Conversion Package
for Mandarin Chinese Based on a New Open Benchmark Dataset,” in
*Proc. Interspeech 2020*, 2020, pp. 1723–1727. [Online]. Available: -
[11]
A. Babu, C. Wang, A. Tjandra, K. Lakhotia, Q. Xu, N. Goyal, K. Singh, P. von
Platen, Y. Saraf, J. Pino
*et al.*, “XLS-R: Self-supervised cross-lingual speech representation learning at scale,”*arXiv preprint arXiv:2111.09296*, 2021. -
[12]
...
[13]
A. Deri and K. Knight, “Grapheme-to-phoneme models for (almost) any
language,” in
*Proceedings of ACL*. Association for Computational Linguistics, 2016, pp. 399–408. -
[14]
B. Peters, J. Dehdari, and J. van Genabith, “Massively multilingual neural
grapheme-to-phoneme conversion,” in
*Proceedings of the First Workshop on Building Linguistically Generalizable NLP Systems*, Sep. 2017. -
[15]
U. Hermjakob, J. May, and K. Knight, “Out-of-the-box universal Romanization
tool uroman,” in
*Proceedings of ACL 2018, System Demonstrations*, Jul. 2018. -
[16]
...
*International Conference on Statistical Language and Speech Processing*. Springer, 2020, pp. 3–19. -
[17]
K. Gorman, L. F. Ashby, A. Goyzueta, A. McCarthy, S. Wu, and D. You, “The
SIGMORPHON 2020 shared task on multilingual grapheme-to-phoneme
conversion,” in
*Proceedings of the 17th SIGMORPHON Workshop on Computational Research in Phonetics, Phonology, and Morphology*, 2020, pp. 40–50. -
[18]
...
with byte representation,” in
*ICASSP*. IEEE, 2020, pp. 8234–8238. -
[19]
K. Vesik, M. Abdul-Mageed, and M. Silfverberg, “One model to pronounce them
all: Multilingual grapheme-to-phoneme conversion with a transformer
ensemble,” in
*Proceedings of the 17th SIGMORPHON Workshop on Computational Research in Phonetics, Phonology, and Morphology*, Jul. 2020. -
[20]
D. R. Mortensen, S. Dalmia, and P. Littell, “Epitran: Precision G2P for
many languages,” in
*Proceedings of LREC*. European Language Resources Association (ELRA), 2018. -
[21]
...
framework,”
*Natural Language Engineering*, vol. 22, no. 6, pp. 907–938, 2016. - [22] “eSpeak NG,” , 2022.
-
[23]
J. L. Lee, L. F. Ashby, M. E. Garza, Y. Lee-Sikka, S. Miller, A. Wong, A. D.
McCarthy, and K. Gorman, “Massively multilingual pronunciation modeling with
WikiPron,” in
*Proceedings of LREC*. European Language Resources Association, 2020, pp. 4223–4228. -
[24]
H. Veisi, M. MohammadAmini, and H. Hosseini, “Toward kurdish language
processing: Experiments in collecting and processing the asosoft text
corpus,”
*Digital Scholarship in the Humanities*, vol. 35, no. 1, pp. 176–193, 2020. - [25] J. Llarena, “Britfone,” , 2017.
- [26] W. Phatthiyaphaibun, “Sprakbanken Swedish pronunciation dictionary,” , 2020.
- [27] J. Morgan, “Santiago Spanish Lexicon ,” , 2017.
- [28] “ipa-dict,” , 2020.
-
[29]
...
in
*Proceedings of the Eighth International Conference on Language Resources and Evaluation (LREC’12)*, 2012, pp. 759–765. - [30] “ISO 639 Code Tables,” , 2022.
-
[31]
R. Sennrich, B. Haddow, and A. Birch, “Neural machine translation of rare
words with subword units,” in
*Proceedings of ACL*. Association for Computational Linguistics, 2016, pp. 1715–1725. -
[32]
...
[34]
M. Řezáčková, J. Švec, and D. Tihelka, “T5G2P: Using Text-to-Text
Transfer Transformer for Grapheme-to-Phoneme Conversion,” in
*Proc. Interspeech 2021*, 2021, pp. 6–10. -
[35]
...
transformer,” in
*Proceedings of the 2021 Conference of the North American Chapter of the Association for Computational Linguistics: Human Language Technologies*, 2021, pp. 483–498. -
[36]
