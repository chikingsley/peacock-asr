...
From our empirical experiment, we found that 65 is the optimal layer numbers for Multilingual ASR~(about 56M params).
...
%The PCA method doesn't outperform weighted sum in either 10min or 1hr set implying that dimensions with larger variance aren't necessarily more crucial for specific downstream tasks. 
%For multiple groups of weighted sum, the performance does not further improve as the number of groups increases.
%From the result, it suggests that 2 groups of weighted sum is the optimal in our case.
%Hence, we set the kernel size of the convolution layer to 5, which is roughly the same as the layers within each weighted sum. i.e. $13/2=6.5$.
...
%On the other hand, it is not as stable as Hierarchical Convolution.
...
Our findings not only resonate with~\cite{zaiem23b_interspeech} that larger downstream models increases the performance but we also show that a better interface design can get further improvements.
