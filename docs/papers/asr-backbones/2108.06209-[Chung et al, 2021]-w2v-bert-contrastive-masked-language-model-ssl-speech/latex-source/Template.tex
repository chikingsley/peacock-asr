...
% Single address.
% ---------------
\name{Yu-An Chung$^{1,2}$\sthanks{Work done during an internship at Google Brain.}, Yu Zhang$^{2}$, Wei Han$^{2}$, Chung-Cheng Chiu$^{2}$, James Qin$^{2}$, Ruoming Pang$^{2}$, Yonghui Wu$^{2}$}
\address{$^{1}$MIT Computer Science and Artificial Intelligence Laboratory\\$^{2}$Google Brain\\
...
\label{sec:intro}
% 6 pages content + 2 pages for references (only)
...
%The core idea of our approach is to make use of masked language modeling~(MLM)~\cite{devlin2019bert}---a self-supervised task for learning contextualized representations of text---for pre-training the ASR networks.
%However, unlike text which is inherently composed of discrete tokens, speech is continuous by nature and thus MLM is not directly applicable to speech.
%To overcome this issue, we adopt wav2vec~2.0
...
\label{sec:related}
% Only discuss the most related work here. Leave less important ones in Introduction.
...
%As studied in Section~IV-$B$ in~\cite{hsu2021hubert}, in order to generate good targets for training the representation learning module, different iterations require running k-means clustering on different numbers of clusters.
%Another key design choice in HuBERT is from which transformer layer should we extract features from to run k-means clustering.
%As analyzed in Section~V-$C$ in~\cite{hsu2021hubert}, using features extracted from different layers to run k-means clustering yields cluster assignments of significantly different quality, which directly affects the pre-training quality.
...
%our experiments show that collapsing solutions do exist for the loss and structure, but contrastive loss plays an essential role in preventing collapsing.
...
\end{table*}
% Define the abstractions and notations first, then describe the actual implementation later.
...
\begin{table*}[htbp]
  ...
  \begin{tabular}{lccccccccccc}
    ...
    & \multirowcell{2}{\\[-7pt]AM\\Size~(B)} & \multirowcell{2}{\\[-7pt]LM\\Size~(B)} & \multicolumn{4}{c}{\bfseries No LM} & \multicolumn{4}{c}{\bfseries With LM} \\
    ...
    & 1.9 & 4.4 & 2.1 & 4.3
    & $-$ & $-$ & 1.9 & 3.9 \\
    % \quad Conformer~XL & 2.1 & 4.9 & 2.3 & 4.9 \\
    \midrule
...
%\myparagraph{{\it w2v-BERT is the only pre-training method }}
%w2v-BERT is the only model that's really better than self-training only
...
%\myparagraph{Intuition behind the necessity of contrastive module}
...
%\myparagraph{Empirical results on the necessity of contrastive module}
...
%For the second reason, we believe that the tasks the contrastive module and the masked prediction module trying to solve could be contradictory from time to time.
%As a concrete example, assume there are two frames~$f_{i}$ and~$f_{j}$~(from the feature encoder's output) that are selected to be masked, and~$c_{i}$ and~$c_{j}$ are their corresponding context vectors extracted by the contrastive module.
%The contradiction occurs when~$f_{i}$ and~$f_{j}$ are quantized to the same code vector~$q$.
%From the point of view of~$f_{i}$, the contrastive loss tries to maximize the similarity between~$c_{i}$ and~$q$, while minimizing the similarity between~$c_{i}$ and a set of quantized vectors corresponding to other masked time steps, including~$f_{j}$, whose quantized vector is also~$q$.

%\todo{discuss the C24 case}

\subsection{Results on Voice Search traffic}
%\todo{can probably update this sentence once we have conformer xxl results}

%\todo{PNMI? do codebook have to be phoneme-like units for mlm to learn good representation?}


%\subsection{Results on low-resource data setting}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccc}
    ...
    %\quad  C12M12~(Ours) & 34.3k~(Voice search) & 6.7  \\
